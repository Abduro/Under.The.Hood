/*
	...
	-----------------------------------------------------------------------------
	Adopted to 'Battery On Wheels' project on 23-Dec-2024 at 05:14:23.992, UTC+4, Batumi, Monday;
*/
...
function _ModalDialog() {
    this.panelId   = "dlg_panel";
    this.contentId = "dlg_content";
    this.titleId   = "dlg_title";
}

_ModalDialog.prototype.Hide = function() {
	var panel_ = document.getElementById(this.panelId);
	if (panel_)
	    panel_.style.display = "none";
	var frame_ = document.getElementById(this.contentId);
	if (frame_)
	    frame_.src = "";
}

_ModalDialog.prototype.Show = function (_URL, _title) {
	var panel_ = document.getElementById(this.panelId);
	if (panel_)
	    panel_.style.display = "block";
	var title_ = document.getElementById(this.titleId);
	if (title_) {
	    title_.innerHTML = _title;
	}
	var frame_ = document.getElementById(this.contentId);
	if (frame_)
	    frame_.src = _URL;
...
function _MessageBox() {
    this.panelId   = "msg_panel";
    this.contentId = "msg_content";
    this.titleId   = "msg_title";
}

_MessageBox.prototype.Hide = function () {
    var panel_ = document.getElementById(this.panelId);
    if (panel_)
        panel_.style.display = "none";
    var frame_ = document.getElementById(this.contentId);
...
_MessageBox.prototype.Show = function (_text, _title, _handler, _action, _obj_id, _buttons) {
    var title_ = document.getElementById(this.titleId);
    if (title_) {
        title_.innerHTML = _title;
    }
    var frame_ = document.getElementById(this.contentId);
    if (frame_)
        frame_.src = "generic_msgbox.aspx?text="
            + _text
            + "&handler=" + _handler
            + "&action=" + _action
            + "&id=" + _obj_id
...
_MessageBox.prototype.Error = function () {
    var title_ = document.getElementById(this.titleId);
    if (title_) {
        title_.innerHTML = "Error";
    }
    var frame_ = document.getElementById(this.contentId);
    if (frame_)
        frame_.src = "../GlobalErrorPage_Compact.aspx";
    var panel_ = document.getElementById(this.panelId);
    if (panel_)
        panel_.style.display = "block";
}
