/*
	Created by Tech_dog (ebontrop@gmail.com) on 14-Dec-2024 at 11:21:59.512, UTC+4, Batumi, Monday
...
const cls_a = ["tab-size-act", "tab-border-act", "tr-sel-act"]; // (1) the size of the tab; (2) the border appearance; (3) bkg fill gradient; (active);
const cls_d = ["tab-size-not", "tab-border-not", "tr-sel-not"]; // (1) the size of the tab; (2) the border appearance; (3) bkg fill gradient; (not-active);
...
function fn_talk_about() {
	...
	var tr_0  = this.document.getElementById("tab_0");
	if (tr_0) {
		for (i_ = 0; i_ < cls_a.length; i_++) { tr_0.classList.add(cls_d[i_]); }
		for (i_ = 0; i_ < cls_d.length; i_++) { tr_0.classList.remove(cls_a[i_]); }
	}

	var tr_1  = this.document.getElementById("tab_1");
	if (tr_1) {
		for (i_ = 0; i_ < cls_d.length; i_++) { tr_1.classList.add(cls_a[i_]); }
		for (i_ = 0; i_ < cls_a.length; i_++) { tr_1.classList.remove(cls_d[i_]); }
	}
}
